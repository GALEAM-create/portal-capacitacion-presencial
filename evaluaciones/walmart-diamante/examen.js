...
const netParams=new URLSearchParams(location.hash.slice(1)),netAccessToken=String(netParams.get("token")||"").trim();
if(netAccessToken)history.replaceState(null,"",location.pathname);
...
async function saveNetResult(){
 ...
 try{
  const response=await fetch(NET_API+"/api/portal/diamante/resultados",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json",...(netAccessToken?{Authorization:`Bearer ${netAccessToken}`}:{})},body:JSON.stringify(netPayload)});
...
beginExam.addEventListener('click',async()=>{
 ...
 try{
  const response=await fetch(NET_API+"/api/portal/session",{credentials:"include",cache:"no-store",headers:netAccessToken?{Authorization:`Bearer ${netAccessToken}`}:{}});
  const data=await response.json().catch(()=>({}));
